...
class CustomDataset(Dataset):
    def __init__(self, file_path, block_size=16, r=0.05):
        ...
        for i in range(num_blocks):
            ...
            # Tính độ trượt theo x và chuẩn hóa ngay trong công thức
            if dx_enc == 0:  # Tránh chia cho 0
                lambda_x = 0
            else:
                if dx_enc > dx:
                    lambda_x = (dx_enc - dx) / dx_enc  # Giá trị trong [0, 1]
                    lambda_x = 0.75 * lambda_x  # Chuẩn hóa vào [0, 0.75]
                else:
                    lambda_x = -(dx - dx_enc) / dx if dx != 0 else 0  # Giá trị trong [-1, 0]
                    lambda_x = 0.75 * lambda_x  # Chuẩn hóa vào [-0.75, 0]

            # Tính độ trượt theo y và chuẩn hóa ngay trong công thức
            if dy_enc == 0:  # Tránh chia cho 0
                lambda_y = 0
            else:
                if dy_enc > dy:
                    lambda_y = (dy_enc - dy) / dy_enc  # Giá trị trong [0, 1]
                    lambda_y = 0.75 * lambda_y  # Chuẩn hóa vào [0, 0.75]
                else:
                    lambda_y = -(dy - dy_enc) / dy if dy != 0 else 0  # Giá trị trong [-1, 0]
                    lambda_y = 0.75 * lambda_y  # Chuẩn hóa vào [-0.75, 0]

            # Cắt tỉa để đảm bảo giá trị nằm trong [-0.75, 0.75]
            lambda_x = max(-0.75, min(0.75, lambda_x))
            lambda_y = max(-0.75, min(0.75, lambda_y))

            # Chuyển đổi lambda_x, lambda_y sang i_x, i_y
            i_x = np.ceil((lambda_x - 0.05 + 0.75) * 100)  
            i_y = np.ceil((lambda_y - 0.05 + 0.75) * 100) 
            
            i_x = max(0, min(100, i_x))
            i_y = max(0, min(100, i_y)) 

            # Lưu block và nhãn (dùng i_x, i_y đã chuẩn hóa)
            self.data_blocks.append(np.array(block_data, dtype=np.float32))
            self.label_blocks.append(np.array([i_x, i_y], dtype=np.float32))
    ...
    def __getitem__(self, idx):
        ...
        y = torch.tensor(self.label_blocks[idx], dtype=torch.long)  # (2,)
        return x, y
...
def save_dataset_to_txt(dataloader, save_path):
    with open(save_path, 'w') as f:
        for batch in dataloader:
            ...
            labels = labels.numpy()  # labels: (batch_size, 2)
            ...
            for i in range(len(labels)):  # Duyệt từng sample trong batch
                ...
                label_x, label_y = labels[i]  # Nhãn của block: [i_x, i_y]

                # Chuyển dữ liệu thành chuỗi
                first_row_str = ','.join(map(str, first_row))
                last_row_str = ','.join(map(str, last_row))

                # Ghi vào file: dòng đầu, dòng cuối, i_x, i_y
                f.write(f"{first_row_str},{last_row_str},{label_x},{label_y}\n")
...
train_loaders, val_loaders, test_loaders = get_dataloaders(
    input_folder, batch_size=8, block_size=16, r=0.05
)
...
for file_name, loader in train_loaders:
    ...
    for i in range(min(5, num_samples)):
        ...
        print(f"    Nhãn (i_x, i_y): {label.numpy()}")
...
for file_name, loader in train_loaders:
    ...
    save_dataset_to_txt(loader, train_path)
    if os.path.exists(train_path):
        print(f"Đã lưu file: {train_path}")
    else:
        print(f"Lỗi: Không thể lưu file {train_path}")
...
for file_name, loader in val_loaders:
    ...
    save_dataset_to_txt(loader, val_path)
    if os.path.exists(val_path):
        print(f"Đã lưu file: {val_path}")
    else:
        print(f"Lỗi: Không thể lưu file {val_path}")
...
for file_name, loader in test_loaders:
    ...
    save_dataset_to_txt(loader, test_path)
    if os.path.exists(test_path):
        print(f"Đã lưu file: {test_path}")
    else:
        print(f"Lỗi: Không thể lưu file {test_path}")
